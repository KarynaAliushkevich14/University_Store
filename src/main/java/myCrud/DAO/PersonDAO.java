...
public class PersonDAO {
    ...
    public void save (Person person){
        jdbcTemplate.update("INSERT INTO person (name,age,email) VALUES (?, ?, ?)", person.getName(), person.getPassword(), person.getEmail());
    }

    //3 method
    //update danych za pomocą konkretnrgo id i osoby
    public void update(int id, Person updatedPerson){
        jdbcTemplate.update("UPDATE person SET name=?, age=?, email=? WHERE id =?", updatedPerson.getName(), updatedPerson.getPassword(), updatedPerson.getEmail(), id);
    }
